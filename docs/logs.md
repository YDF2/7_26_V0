...
- [已完成工作](#已完成工作)
  ...
  - [HD720采集与1:1中心裁剪640x640下采样](#hd720采集与11中心裁剪640x640下采样)
  - [SSH键盘遥控模式接入](#ssh键盘遥控模式接入)
- [未完成工作](#未完成工作)
...
### HD720采集与1:1中心裁剪640x640下采样

#### 工作描述
针对“640x640无黑边输入”的新需求，在保持 HD720 采集的前提下完成 1:1 中心裁剪链路改造：

1. **无黑边目标落地**：将 ZED 路径由 4:3->640x480 调整为 1:1->640x640
2. **几何映射更新**：标定参数按“1280x720中心裁剪到720x720，再缩放到640x640”重新映射
3. **运行尺寸隔离**：Vision 增加 ZED 专用处理尺寸，V4L2 回退保持原有尺寸语义
4. **链路一致性增强**：调试图发送、去畸变、检测输入与坐标归一化统一按处理尺寸执行

#### 相关文档
- [2026-04-15_HD720采集_1比1中心裁剪_640x640下采样实施记录.md](Vision/2026-04-15_HD720采集_1比1中心裁剪_640x640下采样实施记录.md) - 本轮640x640无黑边改造、代码变更与验证建议

#### 状态说明
✅ **已完成** - 代码改造已落地，待实机回归验证边缘目标识别稳定性
...
#### 工作描述
为控制器新增 SSH 终端键盘遥控模式，支持在 Orin 载板上通过 `wasdqe` 手动控制步态与转向，并满足调试场景下的安全与感知要求：

1. **启动参数扩展**：新增 `-f/--controlflag`，可在自动模式与键盘模式间切换
2. **键盘控制接入**：使用 `termios + select` 非阻塞读取终端按键，支持长按持续与超时自动停步
3. **动作触发**：按 `k` 触发 `left_little_kick`，并加入冷却防止连发
4. **安全优先级**：手动模式下始终优先跌倒检测与 FSM 起身流程
5. **头部感知保持**：手动模式中持续执行“找球 -> 跟踪球”
6. **参数配置化**：移动速度、输入超时、踢球冷却统一放入 `config.conf` 可调

#### 相关文档
- [2026-04-18_键盘遥控模式接入与FSM安全策略工作日志.md](Controller/2026-04-18_键盘遥控模式接入与FSM安全策略工作日志.md) - 本轮代码改造、参数说明与验证建议

#### 状态说明
✅ **已完成** - 键盘遥控与安全策略已接入，且已修复 `-f1` 模式下裁判盒识别丢失问题，待实机联调确认步态参数
...
- [2026-04-18_裁判盒队伍号与机器人号参数映射核查日志.md](GameController/2026-04-18_裁判盒队伍号与机器人号参数映射核查日志.md) - 裁判盒显示号码与配置参数、启动参数、回传字段的对应关系核查

### Controller
- [2026-04-18_键盘遥控模式接入与FSM安全策略工作日志.md](Controller/2026-04-18_键盘遥控模式接入与FSM安全策略工作日志.md) - SSH 键盘遥控模式、起身优先级与头部策略接入记录
...
- [2026-04-15_HD720采集_1比1中心裁剪_640x640下采样实施记录.md](Vision/2026-04-15_HD720采集_1比1中心裁剪_640x640下采样实施记录.md) - HD720采集与1:1中心裁剪640x640无黑边改造记录
