...
#### 工作描述
在保持原有视觉架构的基础上，完成摄像头链路收敛与依赖清理：

1. **删除MV相关代码**：移除Camera/Vision中的MV分支与接口逻辑
2. **设备优先级调整**：将Camera打开顺序改为 ZED-mini 优先，失败回退 V4L2
3. **构建依赖清理**：从sensor模块移除MVSDK链接依赖
