...
WalkRemote::WalkRemote(QWidget *parent): range_(200), scale_d(10), scale_xy(5000),
    client_(CONF->get_config_value<string>(CONF->player() + ".address"), CONF->get_config_value<int>("net.tcp")), QMainWindow(parent)
{
    ...
    netinfo_ = QString::fromStdString(CONF->get_config_value<string>(CONF->player() + ".address"))
...
void WalkRemote::closeEvent(QCloseEvent *event)
{
    client_.stop();
    emit closed();
}
